...
async def scrape_github(username: str) -> dict:
    ...
    lang_stats = (
        {l: round((c / total_repos_with_lang) * 100, 1) for l, c in languages.items()}
    ...
    return {
        ...
        "account_created_year": (
            user.get("created_at")[:4] if user.get("created_at") else "Unknown"
        ),
...
async def analyze_profile(github_data: dict) -> dict:
    ...
    if is_org:
        vibe = random.choice([
            "Powering open-source innovation.",
            "Building technology used worldwide.",
            "Driving the future of software.",
            "Advancing modern developer ecosystems."
        ])
    elif top_lang in ["python", "jupyter notebook"]:
        vibe = random.choice([
            "Building intelligent systems and automation.",
            "Focused on data and scalable solutions.",
            "Exploring AI and software craftsmanship."
        ])
    elif top_lang in ["javascript", "typescript"]:
        vibe = random.choice([
    ...
    elif top_lang in ["css", "html"]:
        vibe = random.choice([
            "Making the web a more beautiful place.",
            "Designing pixel-perfect experiences.",
            "Crafting beautiful user interfaces."
        ])
    elif top_lang in ["ruby", "php"]:
        vibe = random.choice([
            "Shipping products, fast.",
            "Building the web's backbone.",
            "Focused on rapid product delivery."
        ])
    else:
        vibe = random.choice([
            "Code explorer navigating the digital frontier.",
...
async def generate_card_html(
    username: str, github_data: dict, analysis: dict
) -> str:
    ...
    skills_html = "".join(
        [f'<span class="badge">{skill}</span>' for skill in analysis.get("top_skills", [])]
    )
